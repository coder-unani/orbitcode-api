...
class Logger:
    ...
    def log_request(self, client_host, client_ip, user_agent, host, method, url):
        self.info(
            f"Request: {method} {url}, Client Host: {client_host}, Client IP: {client_ip}, User-Agent: {user_agent}, Host: {host}"
        )

    def log_response(
        self, client_host, client_ip, user_agent, host, method, url, status_code
    ):
        self.info(
    ...
    def log_exception(self, client_host, client_ip, user_agent, host, exception):
        self.error(
            f"Exception: {str(exception)}, Client Host: {client_host}, Client IP: {client_ip}, User-Agent: {user_agent}, Host: {host}"
        )
