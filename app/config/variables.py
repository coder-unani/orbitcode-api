...
messages["USER_LOGIN_AGREE_FAIL"] = (
    "약관 동의, 개인정보 처리방침 동의, 만 14세 이상 동의가 필요합니다."
)
...
messages["USER_UPDATE_PROFILE_TEXT_SUCC"] = "프로필 이미지 업데이트에 성공했습니다."
messages["USER_UPDATE_PROFILE_TEXT_FAIL"] = "프로필 이미지 업데이트에 실패했습니다."
messages["USER_UPDATE_PROFILE_TEXT_NOT_FOUND"] = "프로필 이미지를 찾을 수 없습니다."
...
messages["USER_PASSWORD_NOT_MATCH"] = "비밀번호가 일치하지 않습니다."
messages["USER_UPDATE_MARKETING_AGREE_SUCC"] = "광고수신 동의 업데이트에 성공했습니다."
messages["USER_UPDATE_MARKETING_AGREE_FAIL"] = "광고수신 동의 업데이트에 실패했습니다."

messages["USER_DELETE_SUCC"] = "회원 탈퇴에 성공하였습니다."
messages["USER_DELETE_FAIL"] = "회원 탈퇴에 실패하였습니다."

messages["VALID_USER_TYPE_SUCC"] = "유효한 회원 유형입니다."
messages["VALID_USER_TYPE_ERR"] = "유효하지 않은 회원 유형입니다."
...
messages["VALID_NICK_LENGTH_ERR"] = (
    "닉네임은 {}자리 이상 {}자리 이하여야 합니다.".format(
        settings.USER_NICKNAME_LENGTH_MIN, settings.USER_NICKNAME_LENGTH_MAX
    )
)

messages["VALID_EMAIL_SUCC"] = "유효한 이메일 주소입니다."
messages["VALID_EMAIL_REQUIRE_ERR"] = "이메일은 필수 입력사항 입니다."
messages["VALID_EMAIL_ALREADY_EXIST"] = "이미 존재하는 이메일 주소입니다."
messages["VALID_EMAIL_INCLUDE_SPACE_ERR"] = "이메일에 공백을 포함할 수 없습니다."
messages["VALID_EMAIL_LENGTH_ERR"] = (
    "이메일 주소는 {}자리 이상 {}자리 이하여야 합니다.".format(
        settings.USER_EMAIL_LENGTH_MIN, settings.USER_EMAIL_LENGTH_MAX
    )
)
...
messages["VALID_PWD_LENGTH_ERR"] = (
    "비밀번호는 {}자리 이상 {}자리 이하여야 합니다.".format(
        settings.USER_PASSWORD_LENGTH_MIN, settings.USER_PASSWORD_LENGTH_MAX
    )
)
messages["VALID_PWD_NOT_INC_WORD_ERR"] = "비밀번호는 반드시 문자를 포함해야 합니다."
messages["VALID_PWD_NOT_INC_NUMBER_ERR"] = "비밀번호는 반드시 숫자를 포함해야 합니다."
messages["VALID_PWD_NOT_INC_SIMBOL_ERR"] = (
    "비밀번호는 반드시 특수문자를 포함해야 합니다."
)
